...
# stream_type = direct
# tuner_count =  4
# friendlyname = fHDHR-xumo
# reporting_firmware_name = fHDHR_XUMO
...
* `method` defaults to `origin` and will pull the xmltv data from xumo. Other Options include `blocks` which is an hourly schedule with minimal channel information. Another option is `zap2it`, which is another source of EPG information. Channel Numbers may need to be manually mapped.
